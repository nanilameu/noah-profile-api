...
def get_profile():
    try:
        # Validar que o corpo da requisição é JSON
        data = request.get_json()

        if not data:
            return jsonify({"success": False, "error": "Missing JSON in request"}), 400

        # Validar que o campo email existe
        email = data.get('email')

        if not email:
            return jsonify({"success": False, "error": "Missing 'email' field in request"}), 400

        # Aqui você pode colocar sua lógica para buscar o perfil
        # No exemplo, vou retornar um mock de perfil fixo

        # Lógica de busca do perfil (substituir por busca real, ex: banco de dados)
        ...
        # Logar o erro no console e retornar resposta
        print(f"Error in /get_profile: {e}")
        return jsonify({"success": False, "error": "Internal server error"}), 500

# Execução local (apenas para desenvolvimento/teste)
